...
test("When", () => {
    ...
    expect(when.match("add", [])).toBe(true)
    expect(when.match("add", ["tag_suggestion"])).toBe(false)


    let when1 = new When([], "add")
    expect(when1.match("add", [])).toBe(true)
    expect(when1.match("add", ["tag_suggestion"])).toBe(false)
    expect(when1.match("home", [])).toBe(false)

    let when2 = new When(["tag_suggestion"], "anypage")
    expect(when2.match("add", [])).toBe(false)
    expect(when2.match("add", ["tag_suggestion"])).toBe(true)

    let when3 = new When(["tag_suggestion"], "add")
    expect(when3.match("add", [])).toBe(false)
    expect(when3.match("add", ["tag_suggestion"])).toBe(true)
    expect(when3.match("home", [])).toBe(false)
    expect(when3.match("home", ["tag_suggestion"])).toBe(false)
})
...
test("HotkeyMap", () => {
    ...
    hotkey_map.set_hotkey("a", new When([], "add"), () => "test2")
    hotkey_map.set_hotkey("ctrl+n", new When([], "add"), () => "test3")

    let test1 = hotkey_map.get_hotkey("home", "ctrl+n", [])

    let test2 = hotkey_map.get_hotkey("add", "a", [])
    let test3 = hotkey_map.get_hotkey("add", "ctrl+n", [])
    let undef = hotkey_map.get_hotkey("add", "ctrl+a", [])
