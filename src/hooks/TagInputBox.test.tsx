...
test("useTagInputBox", async () => {
    ...
    render(<input
        ...
        onChange={(e) => { hook.result.current.suggestionWindowHook.onChangePredicateInputBox(e.target.value) }}
    ...
    expect(hook.result.current.suggestionWindowHook.getFocusedItem()).toEqual("hello")
    await act(async () => { hook.result.current.addFocusedSuggestionItem() })
    ...
    expect(hook.result.current.suggestionWindowHook.getFocusedItem()).toEqual("helloworld")
    await act(async () => { hook.result.current.addFocusedSuggestionItem() })
